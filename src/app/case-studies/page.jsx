            ...
            <section className="py-20">
                <div className="max-w-7xl mx-auto px-6 lg:px-8">
                    <div className="flex flex-wrap gap-2 mb-10 justify-center">
                        <button onClick={() => setFilter("All")} className={`px-4 py-2 rounded-lg text-xs font-semibold transition-all duration-300 ${filter === "All" ? "bg-accent-blue text-white" : "glass text-gray-400 hover:text-white hover:bg-white/[0.07]"}`}>
                            All Projects
                        </button>
                        {categories.map((cat) => (
                            <button key={cat} onClick={() => setFilter(cat)} className={`px-4 py-2 rounded-lg text-xs font-semibold transition-all duration-300 ${filter === cat ? "bg-accent-blue text-white" : "glass text-gray-400 hover:text-white hover:bg-white/[0.07]"}`}>
                                {cat}
                            </button>
                        ))}
                    </div>
                    ...
                    <motion.div layout className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {filtered.map((study, i) => (
                            <FadeIn key={study.slug} delay={i * 0.03}>
                                <Link href={`/case-studies/${study.slug}`} className="glass rounded-xl overflow-hidden group hover:bg-white/[0.05] transition-all duration-300 block h-full">
                                    ...
                                    <div className="p-5">
                                        ...
                                        <p className="text-gray-500 text-xs leading-relaxed line-clamp-2 mb-3">{study.shortDescription}</p>
